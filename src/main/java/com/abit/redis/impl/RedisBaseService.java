...
public class RedisBaseService {
    ...
    @Autowired
    private RedisTemplate<String, Object> transRedisTemplate;

    //=============================common============================
    public void expire(String key, long second) {
        expire(key, second, true);
    }
    ...
    private RedisTemplate<String, Object> getRedisTemplate(boolean isUseTran) {
        return isUseTran ? transRedisTemplate : redisTemplate;
    }
     ...
     * @param item 项 不能为null
     */
    public void hdel(String key, Object item, boolean isUseTran) {
        getRedisTemplate(isUseTran).opsForHash().delete(key, item);
    }
    ...
    public boolean setNx(String key, String val, long seconds){
        Boolean result = redisTemplate.opsForValue().setIfAbsent(key, ProtostuffUtils.serialize(val));
        if (result && seconds > 0){
