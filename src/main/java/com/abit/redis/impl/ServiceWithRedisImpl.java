...
package com.abit.redis.impl;


import com.abit.config.redis.SerializeDeserializeWrapper;
import com.abit.expand.MyLambdaQueryWrapper;
import com.abit.expand.MyPage;
import com.abit.util.ProtostuffUtils;
...
import static com.abit.util.CommonUtil.getMD5x16Str4Collection;
import static com.abit.util.CommonUtil.getMD5x16Str4Entity;
...
public class ServiceWithRedisImpl<M extends BaseMapper<T>, T> extends ServiceImpl<M, T> {
    ...
    private final Long PAGE_CACHE_TIME = 60L * 5;
    ...
    public boolean removeById(Serializable id) {
        ...
        if (result) {
            ...
            redisService4Protostuff.del(key,true);
            delAllPageCache();
            if (RedisLocalData.isInTran()){
    ...
    public MyPage<T> page(T entity, Integer page, Integer size, boolean isUseCache) {
        return page(entity, page, size, isUseCache, defaultDimension, PAGE_CACHE_TIME);
    }

    public MyPage<T> page(T entity, Integer page, Integer size, boolean isUseCache, String dimension) {
        return page(entity, page, size, isUseCache, dimension, PAGE_CACHE_TIME);
    }

    public MyPage<T> page(T entity, Integer page, Integer size, boolean isUseCache, String dimension, Long expireSeconds) {
    ...
    public MyPage<T> page(MyLambdaQueryWrapper<T> wrapper, Integer page, Integer size, boolean isUseCache, String dimension, Long expireSeconds) {
        ...
        String hsetKey = getPageHsetKey() + dimension;
        String key = getMD5x16Str4Collection(wrapper.getSqlRecordList()) + ":" + page + ":" + size;
        Object obj = redisService4Protostuff.hget(hsetKey, key);
        if (obj != null) {
            return (MyPage<T>) ProtostuffUtils.deserialize((byte[]) obj, SerializeDeserializeWrapper.class).getData();
        }
    ...
    public MyPage<T> page(MyLambdaQueryWrapper<T> wrapper, Integer page, Integer size, boolean isUseCache) {
        return page(wrapper, page, size, isUseCache, defaultDimension, PAGE_CACHE_TIME);
    }

    public MyPage<T> page(MyLambdaQueryWrapper<T> wrapper, Integer page, Integer size, boolean isUseCache, String dimension) {
        return page(wrapper, page, size, isUseCache, dimension, PAGE_CACHE_TIME);
    }
